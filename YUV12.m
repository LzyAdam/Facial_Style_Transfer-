...
RGB_0= ycbcr2rgb(yuv0);%转成RGB
 ...
 RGB_4= ycbcr2rgb(yuv4);
  ...
  figure('NumberTitle', 'off', 'Name', 'YUV复合6个不同频段滤波器样子及处理后图像');
...
yuv2= cat(3, Y2, U2, V2);  
toc
figure('NumberTitle', 'off', 'Name', 'YUV2222222图像');
...
%  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  F2=fft2(Y2);          %傅里叶变换
   F1t=real(log(abs(F)+1));   %取模并进行缩放 !!!!!这个加上图像会变成黑红色
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%dispaly the fft result%%%%%%%%%%%
figure('NumberTitle', 'off', 'Name', '99999=>HSV后，V通道的傅里叶频谱图，HSV图，RGB图');
subplot(131),imshow(Y2,[])  ,title('Y2通道频谱图');
subplot(132),imshow(yuv2,[])  ,title('HSV=>FFT三通道图');
...
fr1110=fr11-fr10;
figure('NumberTitle', 'off', 'Name', '1000=>226个不同频段滤波器样子及处理后图像');
...
RGB_6= ycbcr2rgb(yuv6);%转成RGB
 ...
 RGB_10= ycbcr2rgb(yuv10);
  ...
  figure('NumberTitle', 'off', 'Name', 'YUV复合6个不同频段滤波器样子及处理后图像');
subplot(3,4,1);imshow(yuv6,[]);title('D0=10的效果图');
subplot(3,4,2);imshow(T0,[]);title('低通滤波器D0=10');
...
imshow(RGB_67891011,[])  ,title('6频段复合');
% % 
...
%  yuv= cat(3, Y,U,V); 
%  RGB_ = ycbcr2rgb(yuv);%转成RGB
%  figure('NumberTitle', 'off', 'Name', 'YUV图像2RGB_');
%  imshow(RGB_);
%  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%经测试可以完美还原RGB图像%%%%%%%%%%%
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%dispaly the fft result%%%%%%%%%%%
% figure('NumberTitle', 'off', 'Name', 'YUV后，Y通道的傅里叶频谱图，YUV图，RGB图');
% subplot(131),imshow(Y,[])  ,title('Y通道频谱图');
% subplot(132),imshow(yuv,[])  ,title('YUV=>FFT三通道图');
% subplot(133),imshow(RGB_,[])  ,title('RGB图');
% %%%%%%%dispaly the fft result%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
...
%  
% 
%  subplot(231),imshow(RGB_0,[])  ,title('YUV-0频段复合');
%  subplot(232),imshow(RGB_01,[])  ,title('YUV-01频段复合');
%  subplot(233),imshow( RGB_012,[])  ,title('YUV-012频段复合');
%  subplot(234),imshow( RGB_0123,[])  ,title('YUV-0123频段复合');
% subplot(235),imshow( RGB_01234,[])  ,title('YUV-01234频段复合');
%  subplot(236),imshow(RGB_012345,[])  ,title('YUV-012345频段复合');
%  
%  yuv0 = cat(3, fr0,U,V);  
% yuv1= cat(3, fr10, U, V);  
% yuv2 = cat(3, fr21, U, V);  
